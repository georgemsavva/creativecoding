...
This post is a bit of a mess as my method for generating and visualising flows evolved as I was writing it.  But this is art not software engineering so it'll do!   Flow fields, or vector fields, are common tool for producing generative art and there are guides to using them everwhere.  Here I document my particular approach, leading to this image:

![Flow field](flow4.png)
...
So in this post I'll show an approach creating flow fields using R, and then illustrate how they are used in my latest project.
...
In this first simple example I create a matrix of starting positions, $z$, make a vector field $v$ by rotating the current position by just over 90 degrees to make a flow spiralling inward.

Direction is measured by the argument of a complex number, so to keep the velocity of my flow constant I standardise $v$ by diving by $\left|v\right|$.
...
dat[ , v := z*exp(1i*3*pi/5) ] # create vector field
dat[ , v := v/Mod(v) ]
...
Streamlines are created by iterating the flow function.  I'll create a matrix called `pos` to store all the locations for plotting.
...
The functions I'm going to use for my vector fields are *rational functions*, defined by the ratios of two polynomials. So our flow has modulus 1 and argument:

That is, 
$$f(z)=\frac{\prod_{i=1}^{n}(z-a_i)}{\prod_{j=1}^{m}(z-b_j)}$$

I'll illustrate this with a simple polynomial first, turning my last first chunk into a function to plot a particular vector flow, then passing it my function:
...
```{r }
scattermoreplot(pos,width=c(800,600), xlim=c(-1,1), ylim=c(-1,1))
